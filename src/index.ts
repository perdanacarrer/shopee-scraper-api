...
import { closeBrowser } from './services/browserScraper';
...
app.get('/shopee', async (req: Request, res: Response) => {
  try {
    // Query strings are case-sensitive, so accept the common variants
    // people naturally type (storeId, storeID, shopId, ...) instead of
    // only the exact lowercase "storeid"/"dealid".
    const q = req.query;
    const storeid = q.storeid ?? q.storeId ?? q.storeID ?? q.shopid ?? q.shopId ?? q.shop_id;
    const dealid = q.dealid ?? q.dealId ?? q.dealID ?? q.itemid ?? q.itemId ?? q.item_id;
